...
public class IceClient {
	public static void main(String[] args) {
		...
		try {
			...
			//Druga referencja do obiektu
			ObjectPrx base2 = communicator.propertyToProxy("Calc2.Proxy");
			if(base2 == null) { //powyższa opcja się nie uda, gdy nie był wskazany plik konfiguracyjny (--Ice.Config=client.config)
				// 2. Uzyskanie referencji obiektu - to samo co powyżej, ale mniej ładnie
				System.out.println("(using a hard-coded configuration)");
				base2 = communicator.stringToProxy("calc/calc33:tcp -h 127.0.0.2 -p 10010 -z : udp -h 127.0.0.2 -p 10010 -z"); //opcja -z włącza możliwość kompresji wiadomości
			}
				...
				try {
					...
					switch (line) {
						case "add":
							...
							r = obj2.add(7, 8);
							System.out.println("RESULT from calc 33 = " + r);
							break;
						case "add2":
						...
						case "subtract":
							...
							break;
						case "avg":
							double avg = obj1.avg(new long[]{1,2,3,4,5});
